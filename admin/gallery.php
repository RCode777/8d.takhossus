...
if (isset($_POST["submit"])) {
    ...
    $query = mysqli_query($conn, "INSERT INTO `gallery` (`id`, `foto`, `deskripsi`) VALUES (NULL, '$gambar', '$deskripsi'); ");

    if ($query) {
        $alert = '<div class="alert alert-success alert-dismissible fade show" role="alert">
            <strong>Success!</strong> Gallery berhasil di tambah
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
          </div>';
    } else {
        $alert = '<div class="alert alert-danger alert-dismissible fade show" role="alert">
            <strong>Failed!</strong> Gallery gagal di tambah
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
          </div>';
    }
}
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form action="" method="post" enctype="multipart/form-data">
                                ...
                                <br>
                                <button type="submit" name="submit" class="btn btn-primary">Tambah Gallery</button>
                            </form>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="col">
                            <div class="card" style="width: 18rem">
                                ...
                                <div class="card-footer">
                                    <a href="edit-gallery.php?id=<?= $data["id"]; ?>" class="btn btn-primary">Edit</a>
                                    <a href="hapus-gallery.php?id=<?= $data["id"]; ?>" onclick="return confirm('Yakin mau hapus data?')" class="btn btn-danger">Hapus</a>
                                </div>
                            </div>
                        </div> <?php } ?>
                </div>
